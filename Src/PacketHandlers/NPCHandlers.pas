...
begin
  ...
  case NPC.Base.ClientId of
    2363:
      begin
        NPC.NPCFile.Header.Options[0] := 79;
        NPC.NPCFile.Header.Options[1] := 79;
        NPC.NPCFile.Header.Options[2] := 8;
        NPC.NPCFile.Header.Options[3] := 87;
      end;

    2073:
      begin
        NPC.NPCFile.Header.Options[5] := 70;
        NPC.NPCFile.Header.Options[6] := 71;
        NPC.NPCFile.Header.Options[7] := 72;
        NPC.NPCFile.Header.Options[8] := 73;
        NPC.NPCFile.Header.Options[9] := 74;
      end;

    2093:
      begin
        NPC.NPCFile.Header.Options[0] := 68;
        NPC.NPCFile.Header.Options[1] := 67;
        NPC.NPCFile.Header.Options[5] := 8;
      end;

    2111:
      begin
        NPC.NPCFile.Header.Options[0] := 69;
        NPC.NPCFile.Header.Options[1] := 8;
      end;

    2172:
      begin
        NPC.NPCFile.Header.Options[0] := 75;
        NPC.NPCFile.Header.Options[1] := 76;
        NPC.NPCFile.Header.Options[2] := 77;
        NPC.NPCFile.Header.Options[3] := 78;
        NPC.NPCFile.Header.Options[4] := 8;
      end;
    ...
    2339:
      begin
        NPC.NPCFile.Header.Options[0] := 83;
        NPC.NPCFile.Header.Options[1] := 8;
      end;

    2054:
      begin
        NPC.NPCFile.Header.Options[0] := 84;
        NPC.NPCFile.Header.Options[1] := 8;
      end;

    2297:
      begin
        NPC.NPCFile.Header.Options[0] := 80;
        NPC.NPCFile.Header.Options[1] := 8;
      end;
  ...
  begin
    if (NPC.NPCFile.Header.Options[i] > 0) and
      (NPC.NPCFile.Header.Options[i] < 99) then
    begin
...
var
  ...
  NomeGuild: string; // Variável para armazenar o nome da guilda
  src: string; // Variável para armazenar a string final a ser copiada
begin
  ...
  case Option of
    ...
    67:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Parque Ursula', 64);
        Packet.Show := $FFFFDB4D;
      end;
    68:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Basilan [2500 gold]', 64);
        Packet.Show := $FFE6B800;
      end;
    69:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Ir para Regenshein', 64);
        Packet.Show := $FFFFDB4D;
      end;
    70:
      begin
    ...
    71:
      begin
        ...
        begin
          src := 'Remover Guild 1: ' + Guilds[Player.Character.GuildSlot]
            .Ally.Guilds[1].Name;
          // Usando PAnsiChar para garantir que estamos passando um ponteiro para a string
          System.AnsiStrings.StrPLCopy(Packet.Name, src, 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
          Packet.Show := $FFEB5A5A; // Modifica a propriedade Show de Packet
        end
        else
        begin
          System.AnsiStrings.StrPLCopy(Packet.Name, 'Slot 1: Sem Guild', 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
          Packet.Show := $FFEB5A5A; // Modifica a propriedade Show de Packet
        end;
    ...
    72:
      begin
        ...
        begin
          src := 'Remover Guild 2: ' + Guilds[Player.Character.GuildSlot]
            .Ally.Guilds[2].Name;
          // Usando PAnsiChar para garantir que estamos passando um ponteiro para a string
          System.AnsiStrings.StrPLCopy(Packet.Name, src, 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
        ...
        begin
          System.AnsiStrings.StrPLCopy(Packet.Name, 'Slot 2: Sem Guild', 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
          Packet.Show := $FFEB5A5A; // Modifica a propriedade Show de Packet
        end;
      end;
    73:
      begin
        ...
        begin
          src := 'Remover Guild 3: ' + Guilds[Player.Character.GuildSlot]
            .Ally.Guilds[3].Name;
          // Usando PAnsiChar para garantir que estamos passando um ponteiro para a string
          System.AnsiStrings.StrPLCopy(Packet.Name, src, 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
        ...
        begin
          System.AnsiStrings.StrPLCopy(Packet.Name, 'Slot 3: Sem Guild', 64);
          // Copia a string src para Packet.Name com limite de 64 caracteres
          Packet.Show := $FFEB5A5A; // Modifica a propriedade Show de Packet
        end;
      end;
    ...
    79:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Entrar em Leopold', 64);
        Packet.Show := $FFFFDB4D;
      end;

    80:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Leopold', 64);
        Packet.Show := $FFFFDB4D;
      end;

    81:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Entrada Caverna Hekla', 64);
        Packet.Show := $FFFFDB4D;
      end;

    82:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Saida Caverna Hekla', 64);
        Packet.Show := $FFFFDB4D;
      end;

    83:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Lakia', 64);
        Packet.Show := $FFFFDB4D;
      end;
    ...
    85:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name,
          'Loja Especial Wars Zona 1 ', 64);
        Packet.Show := $FFFFDB4D;
      end;
    86:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name,
          'Loja Especial Wars Zona 2 ', 64);
        Packet.Show := $FFFFDB4D;
      end;
    87:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Karak Aéreo', 64);
        Packet.Show := $FFFFDB4D;
      end;
    88:
     begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Teleporte Valhalla', 64);
        Packet.Show := $FFFFDB4D;
     end;
    // 87:
    // begin
    // System.AnsiStrings.StrPLCopy(Packet.Name, 'Inicio da Mina ', 64);
    // Packet.Show := $FFffdb4d;
    // end;
    // 88:
    // begin
    // System.AnsiStrings.StrPLCopy(Packet.Name, 'Teleporte Leopold - Funcional ', 64);
    // Packet.Show := $FFffdb4d;
    // end;
  else
    begin
...
begin
  ...
  if (NPC.Base.ClientId < 2048) or (NPC.Base.ClientId > 3047) then
    Exit;

  // if (NPC.Base.ClientID < 50000) or (NPC.Base.ClientID > 50010) then
  // Exit;
...
var
  Pos: TPosition;
begin
  with Servers[Player.ChannelIndex].NPCS[NpcId].Base.PlayerCharacter.LastPos do
  begin
    Pos.X := X + 1;
    Pos.Y := Y + 1;
  end;
...
begin
  ...
  case Player.OpennedNPC of
    DUNGEON_NPC_MINA_2:
      begin
        if (Player.Party.Members.Count < 10) or (Player.PartyIndex = 0) then
        begin
          Player.SendClientMessage
            ('Necessário estar grupo ou ter 10 membros na party para acessar este npc');
          Player.OpennedNPC := 0;
        ...
        Packet.Dungeon := 30;
        Packet.LevelMin := Servers[Player.ChannelIndex].DGMines2[0].LevelMin;
        Writeln('mostrando dlg da dungeon');
        Writeln(Packet.Dungeon);
    ...
    DUNGEON_NPC_EVG_SUP:
     ...
     2410:
      begin
        Packet.Dungeon := 10;
        Packet.LevelMin := Servers[Player.ChannelIndex].DGPheltas[0].LevelMin;
      end;
           2302:
      begin
        Packet.Dungeon := 6;
        Packet.LevelMin := Servers[Player.ChannelIndex].DGPrisao[0].LevelMin;
      end;
  end;
  Packet.Entering := 1;
  // var i :integer;
...
begin
  if not(Player.Base.PlayerCharacter.LastPos.InRange
    (Servers[Player.ChannelIndex].NPCS[2129].Base.PlayerCharacter.LastPos, 5))
  then
    Exit;
            ...
            1297: // quest de reliquia adormecida
              begin
                ...
                begin
                  ...
                  begin
                    Player.SendClientMessage
                      ('Todos os templos estáo cheios. Tente novamente mais tarde.');
                    Exit;
                  end;
          ...
          case Player.PlayerQuests[QuestIndex].Quest.QuestId of
            408: // exilar para Tiberica
              begin
                ...
                begin
                  Player.SendClientMessage('Você já está em Tiberica.');
                  Exit;
                end;
                if (Player.Character.GuildSlot > 0) then
                begin
                ...
                if (Player.Account.Header.CashInventory.Cash < 30000) then
                begin
                  Player.SendClientMessage
                    ('é necessário possuir [30000] na loja cash para essa ação.');
                  Exit;
                end;
                Dec(Player.Account.Header.CashInventory.Cash, 30000);
                Player.SendPlayerCash;
                Player.Account.Header.Nation := TCitizenship(1);
                Servers[Player.ChannelIndex].SendServerMsg
                  ('O jogador <' + AnsiString(Player.Base.Character.Name) +
                  '> exilou para Tiberica.', 32, 16);
                Player.RemoveQuest(QuestId);
                ZeroMemory(@Player.PlayerQuests[QuestIndex], sizeof(QuestDin));
            ...
            409: // exilar para Odeon
              begin
                ...
                begin
                  Player.SendClientMessage('Você já está em Odeon.');
                  Exit;
                end;
                if (Player.Character.GuildSlot > 0) then
                begin
            ...
            410: // exilio para leonides
              begin
                ...
                begin
                  Player.SendClientMessage('Você já está em Elsinore.');
                  Exit;
                end;
                if (Player.Character.GuildSlot > 0) then
                begin
...
begin
  ...
  // Verifica se o personagem está envolvido na guerra
  for i := 0 to 3 do
  begin
    if (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i]
      .LordMarechal) = String(xGuild.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1]
  ...
  // Verifica a aliança de outros guilds (1 a 3)
  for i := 1 to 3 do
  begin
    if xGuild.Ally.Guilds[i].Index <> 0 then
    begin
      case i of
        1:
          OtherGuild1 := @Guilds[Servers[Player.ChannelIndex].GetGuildSlotByID
            (xGuild.Ally.Guilds[i].Index)];
        2:
      ...
      case i of
        ...
        2:
          ...
          begin
            ...
            OtherGuild2.Ally.Leader := OtherGuild2.Index;
            OtherGuild2.Ally.Guilds[0].Index := OtherGuild2.Index;
  ...
  // Reseta a aliança do próprio guild
  ...
  for i := 0 to 127 do
  begin
    if (xGuild <> nil) and (xGuild.Members[i].Logged) then
    begin
    ...
    if (OtherGuild1 <> nil) and (OtherGuild1.Members[i].Logged) then
    begin
      if Servers[Player.ChannelIndex].GetPlayerByCharIndex
...
begin
  ...
  begin
    ...
    if (String(Nations[Player.Character.Base.Nation - 1]
      .Cerco.Defensoras.Estrategista) = String(OtherGuild1.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1].Cerco.Defensoras.Juiz)
      = String(OtherGuild1.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1]
    ...
    begin
      ...
      ZeroMemory(@Nations[Player.Base.Character.Nation - 1].Cerco.Defensoras,
        sizeof(Nations[Player.Base.Character.Nation - 1].Cerco.Defensoras));
      UpdateNation := True;
    end;
...
begin
  ...
  begin
    Player.SendClientMessage
      ('Você não pode sair de uma aliança sendo o líder dela.');
    Exit;
  end;
  ...
  begin
    if (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i]
      .LordMarechal) = String(xGuild.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i]
      .Estrategista) = String(xGuild.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i].Juiz)
      = String(xGuild.Name)) or
      (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i]
  ...
  if (String(Nations[Player.Character.Base.Nation - 1]
    .Cerco.Defensoras.LordMarechal) = String(xGuild.Name)) or
    (String(Nations[Player.Character.Base.Nation - 1]
    .Cerco.Defensoras.Estrategista) = String(xGuild.Name)) or
    (String(Nations[Player.Character.Base.Nation - 1].Cerco.Defensoras.Juiz)
    = String(xGuild.Name)) or
    (String(Nations[Player.Character.Base.Nation - 1]
...
begin
  if Player.Base.Character.Nation = 4 then
    Exit;

  // Check if current day is Sunday and current time is between 14:00 and 23:00
  if (DayOfWeek(Now) <> 6) or (HourOf(Now) < 13) or (HourOf(Now) >= 23) then
  begin
    Player.SendClientMessage
      ('Você só pode se cadastrar para a guerra castelo na Sexta Feira das 14:00 às 23:00.');
    Exit(False);
  end;
  ...
  begin
    Player.SendClientMessage
      ('Você deve ser o líder da legião para cadastrar na guerra.');
    Exit(False);
  end;
  ...
  begin
    Player.SendClientMessage
      ('Você deve ser o líder da aliança para cadastrar na guerra.');
    Exit(False);
  end;
...
begin
  ...
  // Verificar se o dia é sexta-feira entre 13h e 23h
  if (DayOfWeek(Now) <> 6) or (HourOf(Now) < 13) or (HourOf(Now) >= 23) then
  begin
    Player.SendClientMessage
      ('Você só pode entrar no castelo na Sexta Feira das 13:00 às 23:00.');
    Exit(False);
  end;

  SpawnPoint.Create(3434, 2598);

  // Verificar se o jogador está em uma guilda e nação
  if (Player.Character.GuildSlot = 0) then
  begin
    Player.SendClientMessage
  ...
  // Verificar se a guilda tem direito à guerra
  for i := 0 to 3 do
  begin
    if (String(Nations[Player.Character.Base.Nation - 1].Cerco.Atacantes[i]
      .LordMarechal) = String(xGuild.Name)) or
    ...
    begin
      RigthToWar := True;
      // Definir o SpawnPoint dependendo do índice
      case i of
        0:
      ...
      NationTemp := 7 + i; // Ajusta NationTemp com base no índice
      break; // Sai do loop já que encontrou um direito à guerra
    end;
  end;

  // Verificar defesa se não tiver direito à guerra
  ...
  begin
    ...
    begin
      RigthToWar := True;
      // Ajuste do SpawnPoint de acordo com a defesa
      case i of
        0:
  ...
  // Se não tiver direito à guerra, retornar
  if not(RigthToWar) then
  begin
    Player.SendClientMessage
  ...
  // Iniciar a guerra, se for o líder da guilda e for o líder da aliança
  if (xGuild.GuildLeaderCharIndex = Player.Base.Character.CharIndex) then
  begin
  ...
  // Teleportar o jogador para o castelo se a guerra estiver em progresso
  if (Servers[Player.ChannelIndex].CastleSiegeHandler.WarInProgress) then
  begin
