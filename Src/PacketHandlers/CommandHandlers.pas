...
uses
  Player, Classes, StrUtils, SysUtils, Packets, PacketHandlers;
...
type
  ...
  private
    ...
    class procedure RecallAll(var Player: TPlayer; Data: TStringList);
    class procedure SpawnBoss(var Player: TPlayer; Data: TStringList);
    class procedure SpawnNpc(var Player: TPlayer; Data: TStringList);
  end;
...
{$REGION 'Outros Commandos'}
  case AnsiIndexStr(Command, ['additem', 'equip', 'addexp', 'addgold',
    ...
    'addguildexp', 'addtitle', 'updatetitle', 'removetitle', 'recallall',
    'spawn', 'npc']) of
    0:
    ...
    22:
      Self.RecallAll(Player, CMD);
    23:
      Self.SpawnBoss(Player, CMD);
    24:
      Self.SpawnNpc(Player, CMD);
...
begin
  ...
  try
    ...
    begin
      LogItemUsage5251
        (Format('Foi adicionado um item id: %d quantidade: %d Player %s Na data %s',
        [Index, Quanty, TargetName, DateTimeToStr(Now)]));
    end;

    // Log geral
    LogItem(Format
      ('Foi adicionado um item id: %d quantidade: %d Player %s Na data %s',
      [Index, Quanty, TargetName, DateTimeToStr(Now)]));
  except
    on E: Exception do
    begin
...
class procedure TCommandHandlers.RecallAll(var Player: TPlayer;
  Data: TStringList);
var
  OtherPlayer: PPlayer;
  i: Integer;
begin
  for var l := 0 to 3 do
    ...
    begin
      ...
      OtherPlayer := @Servers[l].Players[i];

      if OtherPlayer.SocketClosed or (OtherPlayer.Socket = 0) or
        (OtherPlayer.Status <> Playing) then
        Continue;

      OtherPlayer.Teleport(Player.Base.PlayerCharacter.LastPos);

    end;

end;

class procedure TCommandHandlers.SpawnBoss(var Player: TPlayer;
  Data: TStringList);
var
  OtherPlayer: PPlayer;
  i: Integer;
begin
  ...
  begin
    ...
    if (Servers[Player.ChannelIndex].Mobs.TMobS[i].IntName <> Data[0].ToInteger)
    then
      Continue
    else
    begin
      TItemFunctions.SpawnMob(Player, Player.Base.PlayerCharacter.CurrentPos, i,
...
class procedure TCommandHandlers.SpawnNpc(var Player: TPlayer;
  Data: TStringList);
var
  OtherPlayer: PPlayer;
  i: Integer;
