...
type
  ...
  public
    ...
    { Mail File Functions }
    class function GetMails(CharacterName: string;
      var Mails: TCharacterMail): Boolean;
    class function GetMail(CharacterName: string; const Index: Int64;
      out Mail: TMail): Boolean; overload;
    class function GetMail(const Mails: TCharacterMail; const Index: Int64;
      out Mail: TMail): Boolean; overload;
    class function GetMailSlot(const Mails: TCharacterMail; const Index: Int64;
      out MailSlot: WORD): Boolean;
    class function SaveMails(CharacterName: string;
      const Mails: TCharacterMail): Boolean;
    class function SaveMail(var Mails: TCharacterMail;
      const Mail: TMail): Boolean;

    { Mail Add e Remove }
    class function AddMail(CharacterName: string; const Mail: TMail): Boolean;
    class function RemoveMail(CharacterName: string; const Index: UInt64)
      : Boolean; overload;
    class function RemoveMail(var Mails: TCharacterMail; const Index: UInt64)
      : Boolean; overload;
    class function RemoveExpiredMails(CharacterName: string): Boolean;

    { Parse Functions }
    class function ParseMail(const Mail: TMail; out Carta: TStructCarta)
      : Boolean; overload;
    class function ParseMail(const Mail: TMail; out Packet: TRecvOpenMailPacket)
      : Boolean; overload;
    class function ParseMail(const MailContent: TMailContent; out Mail: TMail)
      : Boolean; overload;

    { Mail Sends }
    class function SendMailContent(var Player: TPlayer;
      const Mail: TMail): Boolean;
    class function SendRefreshMails(var Player: TPlayer;
      const Mails: TCharacterMail): Boolean;
    class procedure SendUnreadMails(var Player: TPlayer); overload;
    class procedure SendUnreadMails(var Player: TPlayer;
      const Mails: TCharacterMail); overload;

    { Packet Mail Handlers }
    class function SendCharacterMail(var Player: TPlayer;
      var Buffer: array of BYTE): Boolean;
    class function ConfirmSendMail(var Player: TPlayer;
      var Buffer: array of BYTE): Boolean;
    class function RefreshCharacterMail(var Player: TPlayer;
      var Buffer: array of BYTE): Boolean;
    class function OpenMail(var Player: TPlayer;
      var Buffer: array of BYTE): Boolean;
    class function GetMailContent(var Player: TPlayer;
      var Buffer: array of BYTE): Boolean;
  end;
...
begin
  ...
  try
    SQLComp.SetQuery
      ('SELECT count(id) FROM mails WHERE `active` > 0 AND characterId = :owner_chracterId');
...
begin
  ...
  try
    SQLComp.SetQuery
      ('SELECT count(id) FROM mails WHERE `active` > 0 AND characterId = :owner_chracterId AND `checked` = 0');
    SQLComp.AddParameter2('owner_chracterId', Player.Account.Header.AccountId);
    SQLComp.Run();
...
class function TMailFunctions.GetMails(CharacterName: string;
  var Mails: TCharacterMail): Boolean;
begin
  // Result := False;
...
class function TMailFunctions.GetMail(CharacterName: string; const Index: Int64;
  out Mail: TMail): Boolean;
var
...
class function TMailFunctions.GetMail(const Mails: TCharacterMail;
  const Index: Int64; out Mail: TMail): Boolean;
var
  i: DWORD;
begin
  Result := False;
...
class function TMailFunctions.GetMailSlot(const Mails: TCharacterMail;
  const Index: Int64; out MailSlot: WORD): Boolean;
var
  i: WORD;
begin
  Result := False;
...
class function TMailFunctions.SaveMails(CharacterName: string;
  const Mails: TCharacterMail): Boolean;
var
  F: File of TMail;
  FName: string;
...
begin
  ...
  begin
    if not(DirectoryExists(DATABASE_PATH + 'Mails\' +
      UpperCase(CharacterName[1]))) then
      ForceDirectories(DATABASE_PATH + 'Mails\' + UpperCase(CharacterName[1]));

    FName := DATABASE_PATH + 'Mails\' + UpperCase(CharacterName[1]) + '\' +
      CharacterName + '.mail';
  end
  else
  begin
...
class function TMailFunctions.SaveMail(var Mails: TCharacterMail;
  const Mail: TMail): Boolean;
var
  i: WORD;
begin
...
class function TMailFunctions.AddMail(CharacterName: string;
  const Mail: TMail): Boolean;
var
  Mails: TCharacterMail;
  MailCount: WORD;
...
class function TMailFunctions.RemoveMail(CharacterName: string;
  const Index: UInt64): Boolean;
var
  Mails: TCharacterMail;
  MailSlot: WORD;
  MailSizes: DWORD;
...
class function TMailFunctions.RemoveMail(var Mails: TCharacterMail;
  const Index: UInt64): Boolean;
var
  MailSlot: WORD;
  MailSizes: DWORD;
...
class function TMailFunctions.RemoveExpiredMails(CharacterName: string)
  : Boolean;
var
  Mails: TCharacterMail;
  i: WORD;
...
class function TMailFunctions.ParseMail(const Mail: TMail;
  out Carta: TStructCarta): Boolean;
begin
  Result := False;
  ...
  try
    ...
    AnsiStrings.StrPLCopy(Carta.DataRetorno,
      AnsiString(FormatDateTime('yyyy-mm-dd', Mail.DataRetorno)), 20);
...
class function TMailFunctions.ParseMail(const Mail: TMail;
  out Packet: TRecvOpenMailPacket): Boolean;
begin
  ...
  try
    ...
    AnsiStrings.StrPCopy(Packet.DataEnvio,
      AnsiString(FormatDateTime('yyyy-mm-dd hh:mm:ss', Mail.DataEnvio)));
...
class function TMailFunctions.ParseMail(const MailContent: TMailContent;
  out Mail: TMail): Boolean;
begin
  Result := False;
...
class function TMailFunctions.SendMailContent(var Player: TPlayer;
  const Mail: TMail): Boolean;
var
...
class function TMailFunctions.SendRefreshMails(var Player: TPlayer;
  const Mails: TCharacterMail): Boolean;
var
...
class function TMailFunctions.SendCharacterMail(var Player: TPlayer;
  var Buffer: array of BYTE): Boolean;
var
...
begin
  ...
  begin
    ...
    Move(Player.Character.Base.Inventory[Packet.Content.ItemSlot[i]],
      Mail.Item[i], sizeof(TItem));
...
class function TMailFunctions.ConfirmSendMail(var Player: TPlayer;
  var Buffer: array of BYTE): Boolean;
var
...
class function TMailFunctions.RefreshCharacterMail(var Player: TPlayer;
  var Buffer: array of BYTE): Boolean;
var
  Mails: TCharacterMail;
begin
...
class function TMailFunctions.OpenMail(var Player: TPlayer;
  var Buffer: array of BYTE): Boolean;
var
...
class function TMailFunctions.GetMailContent(var Player: TPlayer;
  var Buffer: array of BYTE): Boolean;
var
...
begin
  ...
  if (TItemFunctions.PutItem(Player, Mail.Item[Packet.Slot], 0, True) = 255)
  then
  begin
    Exit;
  end;
