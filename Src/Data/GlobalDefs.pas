...
uses
  Log, ServerSocket, LoginSocket, TokenSocket,
  PlayerData, MiscData, FilesData, Windows, NPC,
  Generics.Collections, GuildData, Nation, Dungeon, Player, Packets;
var
  Key: TPlayerKey;
  ActivePlayers: TDictionary<TPlayerKey, TPlayer>;
  CurrentTime: TDateTime;
  Logger: TLog;
  WebServerClosed: Boolean;
  Servers: ARRAY OF TServerSocket;
  Nations: Array [0 .. 3] of TNationData;
  LoginServer: TLoginSocket;
  TokenServer: TTokenServer;
  PointerPlayer: PPlayer;
  ActivePlayersNow: word;
  DungeonInstances: Array [1 .. 254] of TDungeonInstance;
...
{$ENDREGION}
  ...
  PrimeBoss1Spawned: Boolean = false;
  PrimeBoss2Spawned: Boolean = false;
  PrimeBoss3Spawned: Boolean = false;
  ElterHappening: Boolean = false;
  StartElter: Boolean = false;
  RegisterElterOld: Boolean = false;
  Guilds: ARRAY [1 .. 50] OF TGuild;
  TimeTick: Cardinal;
  InitialAccounts: ARRAY [0 .. 5] OF TCharacterDB;
  MAX_CONNECTIONS: byte = 255;
  SERVER_VERSION: byte; // 290
  SERVER_COUNT: byte;
  MYSQL_SERVER: String;
  MYSQL_PORT: word;
  ...
  { Run Var }
  SERVER_TYPE: string;
  TentativasDict: TDictionary<string, TTentativas>;
  BlockedDict: TDictionary<string, TBlockedIps>;

  ChangeChannelList: TDictionary<DWORD, TChangeChannelToken>;
  ChangeChannelOpcode: TDictionary<string, TPlayerData1>;
  IpList: TList<string>;
  // Logados: TList<TLogados>;
  // PlayerLogged: TLogados;
  // DuelosAtivos: integer;

  MudandoCanal: TDictionary<string, TMudandoCanal>;
  SMudandoCanal: TMudandoCanal;
  // SkillsSents: TDictionary<String, TSkillEnviada>;
  // SkillsSent: TSkillEnviada;
  // ChangeChannelList: TDictionary<DWORD, TChangeChannelToken>;
  // DropItemF12: WORD = 1;
  Old_Exp: word = 1;
  Mensagem_onlogin: string;
  Dungeon_Status: word = 0;
  Dungeon_Modificada: word = 0;
  Dungeon_Valor_Status: word = 1;
  Dungeon_Valor_Normal: word = 5000;
  Dungeon_Valor_Dificil: word = 25000;
  Dungeon_Valor_Elite: word = 50000;
  Dungeon_Valor_Infernal: Integer = 1000000;
  PvP_Min_Level: word = 10;
  PvP_Debuff_Status: word = 1;
  PvP_Exp_Guild: word = 20;
  Elter_Status: word = 1;
  Elter_Max_Players: word = 2;
  Elter_Inatividade_Kick: word = 1;
  Elter_Inatividade_Kick_Tempo: word = 1;
  Elter_Inatividade_Kick_Avisos: word = 1;
  Elter_Min_Level: word = 1;
  Elter_PvP_Debuff: word = 1;
  Elter_PvP_Acquire_Solo: word = 1;
  Elter_PvP_Acquire_Raid: word = 1;
  Elter_PvP_Acquire_Group: word = 1;
  Elter_Acquire_PvP: word = 1;
  Elter_Honor_Value: word = 20;
  Elter_Item_Perkill_Status: word = 1;
  Elter_Item_Perkill_Item: word = 11285;
  Elter_Item_Perkill_Item_Quantidade: word = 2;
  Premio_Elter_Vitoria: word = 4456;
  Premio_Elter_Quantidade_Vitoria: word = 3;
  Premio_Elter_Empate: word = 4456;
  Premio_Elter_Quantidade_Empate: word = 2;
  Premio_Elter_Derrota: word = 4456;
  Premio_Elter_Quantidade_Derrota: word = 1;
  LEVEL_CAP: word = 50;
  VALOR_LILOLA: word = 50000;
  MAX_PRAN_LEVEL: word = 20;
  ...
  EXP_MULTIPLIER: word = 1;
  HONOR_PER_KILL: word = 120;
  PVP_ITEM_DROP_TAX: word = 100;
  SKULL_MULTIPLIER: byte = 1;
  DUEL_TIME_WAIT: word = 9;
  RELIQ_EST_TIME: word = 2;
  INC_HONOR_RELIQ_LEVEL: word = 250;
  RATE_EFFECT5: word = 10;
  DISTANCE_TO_WATCH: byte = 50;
  DISTANCE_TO_FORGET: byte = 60;
  MAX_PERCENTAGE: Integer;
  TAXA_AUXILIAR: Integer;
  TAXA_AUXILIAR_DUPLO: Integer;
  TAXA_AUXILIAR_MISS: Integer;
  MapNames: array [0 .. 11] of String = (
    'Mapa de Testes',
    'Regenshein', // ID 1
    'Verband', // ID 2
    '', // ID 3 (Gap)
    ...
    'Parque Ursula', // ID 5
    'Caverna Hekla', // ID 6
    'Estrada Bifurcada Halperin', // ID 7
    'Deserto de Sigmund', // ID 8
    'Mina Lenfer', // ID 9
    '', // ID 10 (Gap)
    'Cahill' // ID 11
  );
  Nacoes: array [0 .. 5] of string = (
    'Tibérica',
    'Odeon',
    'Elsinore',
    'Leopold',
...
const
  Inicio_Guardas_Royal = 3391;
  Fim_Guardas_Royal = 3498;
  MAX_BYTE_SIZE = 255;
  MAX_WORD_SIZE = 65535;
  MIN_BYTE_SIZE = 0;
  MIN_WORD_SIZE = 0;
  ACCEPT_CONNECTIONS_DELAY = 5; // Delay em miliseconds

  MAX_SPAWN_ID = 60000;
  AI_DELAY_MOVIMENTO = 4000;
  MIN_DELAY_ATTACK = 150;
  // (ms)                 //aqui delay de ataque padrão acho
  MIN_DELAY_ATTACK_Skill = 1000;
  // antes 500     //aqui que muda delay de skill acho
  ...
  DUNGEON_NPC_PHELTAS = 2410;
  { Refinamento }
  ChancesOfRefinament: Array [0 .. 14] of byte = (100, 100, 100, 100, 100, 100,
    100, 100, 100, 100, 100, 100, 100, 100, 100);
  { Vaizan enchant ranges }
  VaizanP_Set: array [0 .. 8] of Integer = (5352, 5355, 5358, 5361, 5364, 5367,
    5370, 5373, 5376);

  VaizanM_Set: array [0 .. 8] of Integer = (5351, 5354, 5357, 5360, 5363, 5366,
    5369, 5372, 5375);
  ...
  VaizanP_Acc: array [0 .. 14] of Integer = (5397, 5400, 5403, 5406, 5409, 5412,
    5415, 5418, 5421, 5424, 5427, 5430, 5433, 5448, 5451);
  // VaizanP_Acc: array [0 .. 2] of Integer = (5397, 5397, 5398);

  VaizanM_Acc: array [0 .. 14] of word = (5396, 5399, 5402, 5405, 5408, 5411,
    5414, 5417, 5420, 5423, 5426, 5429, 5432, 5447, 5450);

  VaizanG_Acc: array [0 .. 14] of word = (5395, 5398, 5401, 5404, 5407, 5410,
    5413, 5416, 5419, 5422, 5425, 5428, 5431, 5446, 5449);

  Level_10_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4630, 4631);
  Level_20_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4632, 4633);
  Level_30_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4634, 4635);
  Level_40_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4636, 4638);
  Level_50_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4642, 4644);
  Level_60_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4644, 4636);
  Level_70_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4642, 4645);
  Level_80_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4649, 4647);
  Level_90_Destroy_Range: Array [0 .. 5] of word = (5100, 5080, 5081, 5082,
    4649, 4647);
  ...
  SKILL_POINT_X = 7; // muda aqui pra alterar o status
  STATUS_POINT_X = 10;
  { Conjunt DEF }
...
const
  GuildExpList: Array [0 .. 9] of Comp = (0, 22000, 64750, 156500, 496000,
    1066400, 3199200, 6398400, 10664000, 15996000);
{$ENDREGION}
{$REGION 'ItemType'}
  ITEM_TYPE_FACE = 0;
  ITEM_TYPE_HAIR = 1;
  ...
  ITEM_TYPE_SCROLL_DIMENSIONAL = 206;
...
//ThreadPools := TThreadPool.Create;
ActivePlayers := TDictionary<TPlayerKey, TPlayer>.Create;
ChangeChannelList := TDictionary<DWORD, TChangeChannelToken>.Create;
ChangeChannelList.Clear;
MudandoCanal := TDictionary<string, TMudandoCanal>.Create;
MudandoCanal.Clear;
TentativasDict := TDictionary<string, TTentativas>.Create;
TentativasDict.Clear;
BlockedDict := TDictionary<string, TBlockedIps>.Create;
BlockedDict.Clear;
ChangeChannelOpcode := TDictionary<string, TPlayerData1>.Create;
ChangeChannelOpcode.Clear;
