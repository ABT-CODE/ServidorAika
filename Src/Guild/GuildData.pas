...
type
  ...
  public
    ...
    function CreateGuild(GuildName: ARRAY OF AnsiChar;
      GuildNation, ChannelId: Integer; Party: PParty;
      SelfPlayer: pointer): Boolean;
...
begin
  ...
  try
    ...
    SQLComp.SetQuery
      ...
      ' %d, %d, %d, %d, %d, %s, %s,' + ' %s, %s, %d, %d, %d, %d, %d, %d,' +
      ' %d, %s, %d, %s,' + ' %d, %s, %d, %s,' + ' %d, %d)',
      [Self.Slot, QuotedStr(String(Self.Name)), Self.Nation, Self.Exp,
      Self.Level, Self.TotalMembers, Self.BravurePoints, Self.SkillPoints,
      Self.Promote.ToInteger, QuotedStr(String(Self.Notices[0].Text)),
      QuotedStr(String(Self.Notices[1].Text)),
      QuotedStr(String(Self.Notices[2].Text)), QuotedStr(String(Self.Site)),
      Self.RanksConfig[0], Self.RanksConfig[1], Self.RanksConfig[2],
      Self.RanksConfig[3], Self.RanksConfig[4], Self.Ally.Leader,
      Self.Ally.Guilds[0].Index, QuotedStr(String(Self.Ally.Guilds[0].Name)),
    ...
    SQLComp.Run(False);
    SQLComp.Query.SQL.Clear;
    SQLComp.Query.SQL.Add
      (Format('SELECT id FROM guilds WHERE leader_char_index=%d',
      [Self.GuildLeaderCharIndex]));
    SQLComp.Run(True);
    ...
    begin
      ...
      SQLComp.SetQuery
        ...
        QuotedStr(String(Member.Name)), Member.Rank, Member.ClassInfo,
        Member.Level, Member.Logged.ToInteger, Member.LastLogin]));
      ...
      begin
        for I := Low(xGuild.Members) to High(xGuild.Members) do
          if (xGuild.Members[I].CharIndex > 0) then
            for P := Low(Servers) to High(Servers) do
              if Servers[P].GetPlayerByCharIndex(xGuild.Members[I].CharIndex,
                Player) then
              begin
                Player.SendPacket(Packet, Size);
                Break;
...
begin
  ...
  Result.Invite := (Value and $01) <> 0;
  Result.Kick := (Value and $02) <> 0;
  Result.UseGuildSkill := (Value and $04) <> 0;
  Result.EditNotices := (Value and $10) <> 0;
  Result.OpenGWH := (Value and $40) <> 0;
  Result.UseGWH := (Value and $80) <> 0;
end;
